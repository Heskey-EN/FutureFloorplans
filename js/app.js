import {
  ...
  distance, getWall, makeOpening, metres, newId, normaliseBox, outlineFromBoxes, pointAlong, polygonArea, squareMetres, syncWalls, wallLength
} from './geometry.js';
...
const elements = {
  ...
  dialogMessage: $('#dialogMessage'), dialogConfirm: $('#dialogConfirm')
};
...
function drawDimensions(storey) {
  ...
  ctx.save(); ctx.font = '700 10px ui-sans-serif, system-ui'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  for (const wall of storey.walls) {
    ...
    const label = `${length.toFixed(2)} m`; ctx.fillStyle = 'rgba(255,255,255,.9)'; const width = ctx.measureText(label).width + 8; ctx.fillRect(midX + offX - width / 2, midY + offY - 8, width, 16); ctx.fillStyle = '#41545f'; ctx.fillText(label, midX + offX, midY + offY);
  }
  ctx.restore();
}
...
function drawBoxOverlay(box, { pending = false } = {}) {
  ...
  const label = `${box.width.toFixed(2)} m × ${box.depth.toFixed(2)} m`; ctx.font = '800 11px ui-sans-serif, system-ui'; const labelWidth = ctx.measureText(label).width + 16; const labelX = topLeft.x + width / 2; const labelY = Math.max(15, topLeft.y - 15); ctx.fillStyle = '#102332'; ctx.fillRect(labelX - labelWidth / 2, labelY - 11, labelWidth, 22); ctx.fillStyle = '#fff'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(label, labelX, labelY); ctx.restore();
}
...
function renderCanvas() {
  ...
  ctx.clearRect(0, 0, rect.width, rect.height); drawGrid(rect.width, rect.height); drawPlan(); drawPreview(); drawBoxPreview();
...
function renderInspector() {
  ...
  if (state.pendingBox) {
    ...
    elements.inspector.innerHTML = `
      ...
      <div class="empty-state"><b>${isExtension ? 'Make a complex shape quickly.' : 'Give the rough drag its exact dimensions.'}</b>${isExtension ? 'This box must touch the existing footprint along an edge. It will be merged into one clean external outline.' : 'The rough box only decides the position. Type the measured width and depth from your laser.'}</div>
...
function instructions() {
  if (state.tool === 'box') return activeStorey().outline.length ? 'Box footprint replaces the active storey · drag rough bounds, then type the measured size' : 'Drag rough footprint bounds · enter the measured width and depth in metres';
  if (state.tool === 'extend') return activeStorey().boxes?.length ? 'Drag an adjoining box · type the measured size · it merges into the footprint' : 'Use Box footprint first, then add touching extension boxes';
  if (state.tool === 'draw') return activeStorey().is_closed ? 'Outline closed · switch to Select to classify walls and add openings' : (activeStorey().outline.length ? 'Tap next internal corner · tap first point to close · drag to size a wall' : 'Tap the first internal corner to begin · drag to size · hold Shift for free angle');
  ...
  return 'Tap a wall or opening to inspect it · drag to pan · pinch to zoom';
}
...
function pointerDown(event) {
  event.preventDefault(); canvas.setPointerCapture?.(event.pointerId); const point = clientPoint(event); state.pointers.set(event.pointerId, point);
  ...
  state.drag = { id: event.pointerId, start: point, view: { x: state.view.x, y: state.view.y }, moved: false };
}
...
function pointerMove(event) {
  ...
  if (state.tool === 'draw') state.drawPreview = constrainedPoint(point, event);
...
function setTool(tool) { state.tool = tool; state.selection = null; state.boxPreview = null; render(); }
...
function cancelBox() { state.pendingBox = null; state.boxError = ''; render(); }
